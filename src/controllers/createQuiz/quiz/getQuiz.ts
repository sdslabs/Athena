...
const formatTime = (timestamp: any) => {
  return [padTo2Digits(timestamp.getHours()), padTo2Digits(timestamp.getMinutes())].join(':')
}
  ...
  try {
    ...
    if (!quiz) {
      ...
      const quizDetails = {
        ...
        startDate: quizMetadata?.startDateTimestamp
          ? formatDate(quizMetadata?.startDateTimestamp)
          : '',
        startTime: quizMetadata?.startDateTimestamp
          ? formatTime(quizMetadata?.startDateTimestamp)
          : '',
        ...
        duration: quizMetadata?.duration
          ? `${Math.floor(quizMetadata?.duration / 60)
              .toString()
              .padStart(2, '0')}:${(quizMetadata?.duration % 60).toString().padStart(2, '0')}`
          : '',
      ...
      return res
        .status(200)
        .send({ message: 'Quiz found', quizDetails, registrationForm, sectionsDetails })
    }
  } catch (error: unknown) {
    sendFailureResponse({
      res,
